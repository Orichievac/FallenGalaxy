...
public class DebugSystems extends ScriptModel {
	// ------------------------------------------------------- CONSTANTES -- //
	// -------------------------------------------------------- ATTRIBUTS -- //
	// ---------------------------------------------------- CONSTRUCTEURS -- //
	// --------------------------------------------------------- METHODES -- //
	...
	public JSONStringer execute() {
		...
		{
			// Test si le proprio du système existe
			if(system.getIdOwner()!=0 && DataAccess.getPlayerById(system.getIdOwner())==null)
			{
				logger="Un système ("+system.getId()+") n'a pas de propriétaire existant.";
				LoggingSystem.getActionLogger().info(logger);
				msg+=logger+"<br/>";

				system.delete();
			}
			// Test si le système est dans un secteur inexistant
			else if(system.getIdOwner()!=0 && DataAccess.getAreaById(system.getIdArea())==null)
			{
				logger=system.getOwner().getLogin()+" ("+system.getOwner().getId()+")"+
					" à un système dans un secteur inexistant :"+system.getIdArea();
				LoggingSystem.getActionLogger().info(logger);
				msg+=logger+".<br/>";
