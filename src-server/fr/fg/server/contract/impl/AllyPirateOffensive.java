...
public class AllyPirateOffensive extends AllyContractModel implements GameEventListener {
	// ------------------------------------------------------- CONSTANTES -- //
	...
	public AllyPirateOffensive() throws Exception {
	...
	public void initialize(Contract contract) {
		...
		// 20 par joueur de level joueur ou +1 à détruire
		 int kills = (int) (contract.getDifficulty()*3);
	...
	public void finalize(Contract contract) {
		...
		LoggingSystem.getActionLogger().info("Mission AllyPirateOffensive terminée pour : "+ally.getName());
	...
	public void onGameEvent(GameEvent event) throws Exception {
		...
		if (event instanceof AfterBattleEvent) {
			...
			List<Contract> contracts=null;
			if(player.getAlly()!=null && player.getRunningContractsByType("PirateOffensive").size() ==0)
			{
				contracts = player.getAlly().getRunningContractsByType(getType());
			}
			// Vérifie que la flotte qui attaque est une flotte pirate
			if (attackingFleet.getSkillLevel(
					Skill.SKILL_PIRATE) == -1)
				return;
			
			// Vérifie que la flotte qui défend n'est pas une flotte pirate
			...
			// Met à jour la liste des flottes du joueurs
			UpdateTools.queuePlayerFleetsUpdate(player.getId(), false);
			
			// Met à jour le secteur
			UpdateTools.queueAreaUpdate(attackingFleet.getIdCurrentArea());
	...
	public void createReward(Contract contract) {
		...
		ContractHelper.addCreditReward(contract,
			(long) (contract.getAttendees().get(0).getAlly().getPoints()*1.2));
	}
	...
	public String getDetailedGoal(Contract contract, Ally ally) {
		if (contract.getState().equals(Contract.STATE_RUNNING)) {
			StringBuffer buffer = new StringBuffer();
				...
				buffer.append(" flottes non pirates de puissance supérieure ou égale à votre niveau joueur avec des flottes pirates.");
				buffer.append("<br/>");
				buffer.append("Si votre niveau est supérieur à 40, vous devez détruire les flottes de puissance 40 avec des puissances 40 maximum.");
				...
				buffer.append(getTotalKills(contract));
				buffer.append("</b> ");
				buffer.append("flottes non pirates détruites.");
				buffer.append("<br/>");
