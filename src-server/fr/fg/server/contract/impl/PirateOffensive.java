...
public class PirateOffensive extends PlayerContractModel implements GameEventListener {
	// ------------------------------------------------------- CONSTANTES -- //
	...
	private static final PlayerLocationConstraints LOCATION_CONSTRAINTS =
		PlayerLocationConstraintsFactory.getPointsLocationConstraints(15000, 3);
	
	
	// ---------------------------------------------------- CONSTRUCTEURS -- //
	
	public PirateOffensive() throws Exception {
	...
	public void finalize(Contract contract) {
		...
		LoggingSystem.getActionLogger().info("Mission PirateOffensive terminée pour : "+player.getLogin());
	...
	public void createReward(Contract contract) {
		...
		ContractHelper.addCreditReward(contract,
			(long) (contract.getAttendees().get(0).getPlayer().getPoints()*1.15));
	}
