...
public class UpdateTools {
	...
	public static void queueEffectUpdate(Effect effect, int sourcePlayer,
		...
		List<Integer> idPlayers = ConnectionManager.getInstance(
			).getPlayersByArea(effect.getIdArea());
		Area area = effect.getArea();
		
		for (Integer idPlayer : idPlayers) {
			if (idPlayer == sourcePlayer)
				continue;
			...
			if (ConnectionManager.getInstance().isConnected(idPlayer)) {
				...
				if (player.isLocationVisible(area, effect.getX(),
						effect.getY(), effect.getRadius(), allySystems,
						allyFleets, allyWards, allySpaceStations,
						allyStructures)) {
					UpdateManager.getInstance().addUpdate(player.getId(),
						Update.getEffectUpdate(effect), highPriority);
					break;
				}
			}
		}
	...
	public static void queueServerShutdownUpdate() {
		HashSet<Integer> connectedPlayers =
			new HashSet<Integer>(ConnectionManager.getInstance().getConnectedPlayers());
		
		Update update = Update.getServerShutdownUpdate(
			ServerController.getShutdownDate() == -1 ? -1 :
			Math.max(0, (int) (ServerController.getShutdownDate() - Utilities.now())));
		
		for (int idPlayer : connectedPlayers)
			UpdateManager.getInstance().addUpdate(idPlayer, update);
	}
	...
	public static void queueNewAllyNewsUpdate(Ally ally, int idSourcePlayer, boolean highPriority) {
		...
		synchronized (members) {
			for (Player member : members) {
				if (member.getId() != idSourcePlayer &&
						ConnectionManager.getInstance().isConnected(member.getId())) {
					UpdateManager.getInstance().addUpdate(
						member.getId(), Update.getNewNewsUpdate(), highPriority);
				}
			}
		}
	}
	...
	public static void queueNewMessageUpdate(int idPlayer) {
		if (ConnectionManager.getInstance().isConnected(idPlayer))
			UpdateManager.getInstance().addUpdate(
				idPlayer, Update.getNewMessageUpdate());
	}
	...
	public static void queueAllyUpdate(int idPlayer, long lastUpdate, boolean highPriority) {
		if (ConnectionManager.getInstance().isConnected(idPlayer))
			UpdateManager.getInstance().addUpdate(idPlayer,
				Update.getAllyUpdate(lastUpdate), highPriority);
	}
	...
	public static void queueChatChannelsUpdate(int idPlayer, boolean highPriority) {
		if (ConnectionManager.getInstance().isConnected(idPlayer))
			UpdateManager.getInstance().addUpdate(
				idPlayer, Update.getChatChannelsUpdate(), highPriority);
	}
	...
	public static void queueContractsUpdate(int[] idPlayers, boolean highPriority) {
		for (int idPlayer : idPlayers)
			if (ConnectionManager.getInstance().isConnected(idPlayer)) {
				Player player = DataAccess.getPlayerById(idPlayer);
				UpdateManager.getInstance().addUpdate(player.getId(),
					Update.getPlayerContractsUpdate(), false);
				UpdateManager.getInstance().addUpdate(player.getId(),
					Update.getContractStateUpdate(), highPriority);
			}
	}
	...
	public static void queueXpUpdate(int idPlayer, boolean highPriority) {
		if (ConnectionManager.getInstance().isConnected(idPlayer))
			UpdateManager.getInstance().addUpdate(
				idPlayer, Update.getXpUpdate(), highPriority);
	}
	...
	public static void queueNewEventUpdate(int idPlayer, boolean highPriority) {
		if (ConnectionManager.getInstance().isConnected(idPlayer))
			UpdateManager.getInstance().addUpdate(idPlayer,
				Update.getNewEventUpdate(), highPriority);
	}
		...
		synchronized (players) {
			for (Player player : players) {
				if (ConnectionManager.getInstance().isConnected(player.getId()))
					UpdateManager.getInstance().addUpdate(
							player.getId(), Update.getNewEventUpdate(), highPriority);
			}
		}
	}
	...
	// Envoie une mise à jour des flottes du joueur
	public static void queuePlayerFleetsUpdate(int idPlayer, boolean highPriority) {
		if (ConnectionManager.getInstance().isConnected(idPlayer))
			UpdateManager.getInstance().addUpdate(idPlayer,
				Update.getPlayerFleetsUpdate(), highPriority);
	}
	...
	public static void queuePlayerFleetUpdate(int idPlayer, int idFleet, boolean highPriority) {
		if (ConnectionManager.getInstance().isConnected(idPlayer))
			UpdateManager.getInstance().addUpdate(idPlayer,
				Update.getPlayerFleetUpdate(idFleet), highPriority);
	}
	...
	public static void queuePlayerGeneratorsUpdate(int idPlayer, boolean highPriority) {
		if (ConnectionManager.getInstance().isConnected(idPlayer))
			UpdateManager.getInstance().addUpdate(idPlayer,
				Update.getPlayerGeneratorsUpdate(), highPriority);
	}
	...
	public static void queuePlayerSystemsUpdate(int idPlayer, boolean highPriority) {
		if (ConnectionManager.getInstance().isConnected(idPlayer))
			UpdateManager.getInstance().addUpdate(idPlayer,
				Update.getPlayerSystemsUpdate(), highPriority);
	}
	...
	public static void queueProductsUpdate(List<Player> players, boolean highPriority) {
		synchronized (players) {
			for (Player player : players) {
				if (ConnectionManager.getInstance().isConnected(player.getId()))
					UpdateManager.getInstance().addUpdate(player.getId(),
						Update.getProductsUpdate(), highPriority);
			}
		}
	}
	
	// Envoie une mise à jour du secteur consulté par un ou plusieurs joueurs
	...
	public static void queueAreaUpdate(boolean highPriority, Player player, Player... players) {
		if (ConnectionManager.getInstance().isConnected(player.getId()) &&
				player.getIdCurrentArea() != 0)
			UpdateManager.getInstance().addUpdate(player.getId(),
				Update.getAreaUpdate(), highPriority);
		
		for (Player player2 : players) {
			if (ConnectionManager.getInstance().isConnected(player2.getId()) &&
					player.getIdCurrentArea() != 0)
				UpdateManager.getInstance().addUpdate(player2.getId(),
					Update.getAreaUpdate(), highPriority);
		}
	}
	
	// Envoie une mise à jour du secteur consulté par un ensemble de joueurs
	...
	public static void queueAreaUpdate(int idArea, int sourcePlayer,
			Point... locations) {
    ...
    queueAreaUpdate(idArea, sourcePlayer,
                    locations.toArray(new Point[locations.size()]));
}
	
	// Met à jour l'affichage des joueurs connectés sur un secteur
	// Attention : fonction très gourmande !
	// Essayer au maximum d'utiliser le brouillard de guerre en indiquant les
	// points qui ont été modifiés (locations) afin de réduire le nombre de
	// mises à jour nécessaires
	public static void queueAreaUpdate(int idArea, int sourcePlayer,
			boolean highPriority, Point... locations) {
		List<Integer> idPlayers = ConnectionManager.getInstance(
				).getPlayersByArea(idArea);
		...
		for (Integer idPlayer : idPlayers) {
			...
			if (locations.length > 0) {
				...
				for (Point location : locations) {
					if (player.isLocationVisible(area, location.x,
							location.y, allySystems, allyFleets,
							allyWards, allySpaceStations, allyStructures)) {
	...
	public static String formatUpdates(Player player, List<Update> updates) {
		...
		synchronized (updates) {
			updates:for (int i = 0; i < updates.size(); i++) {
				...
				// Elimine les mises à jour redondantes
				for (int j = 0; j < i; j++)
					if (update.equals(updates.get(j)))
						continue updates;

				try {
					String data = update.getData(player);
