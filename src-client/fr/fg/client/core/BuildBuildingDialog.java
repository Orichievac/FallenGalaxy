...
public class BuildBuildingDialog extends JSDialog
		implements SelectionListener, ClickListener, ActionCallback {
	// ------------------------------------------------------- CONSTANTES -- //
	
	private final static String[][] FILTERS = {{
		// Tous
		BuildingData.CIVILIAN_INFRASTRUCTURES, BuildingData.CORPORATIONS,
		BuildingData.TRADE_PORT,
		BuildingData.EXPLOITATION0, BuildingData.EXPLOITATION1,
		BuildingData.EXPLOITATION2, BuildingData.EXPLOITATION3,
		BuildingData.REFINERY, BuildingData.EXTRACTOR_CENTER,
		BuildingData.LABORATORY, BuildingData.RESEARCH_CENTER,
		BuildingData.SPACESHIP_YARD, BuildingData.DEFENSIVE_DECK,
		BuildingData.STOREHOUSE, BuildingData.FACTORY
	}, {
		// Construction
		BuildingData.CIVILIAN_INFRASTRUCTURES, BuildingData.CORPORATIONS,
		BuildingData.TRADE_PORT,
	...
	public BuildBuildingDialog(ResourcesManager resourcesManager,
	...
	public void show(PlayerStarSystemData systemData, int buildSlot) {
		...
		for (String type : BuildingData.BUILDINGS)
		...
		if (buildingsCount >= systemData.getBuildingLand())
			filterComboBox.setSelectedIndex(FILTER_UPGRADES);
		else
			filterComboBox.setSelectedIndex(FILTER_ALL);
		...
		if (sender == okBt) {
			...
			if (buildingUI.toString().contains("unavailable")) {
				JSOptionPane.showMessageDialog(
					"Vous ne pouvez pas lancer la construction de ce bâtiment.",
					"Erreur",
				...
				switch(buildSlot) {
					case 0:
						...
						if (systemData.getBuildsCount() > 1) {
							...
							params.put("queue_level", String.valueOf(systemData.getBuildAt(1).getLevel()));
							if(systemData.getBuildsCount() > 2) {
								params.put("third_id", String.valueOf(systemData.getBuildAt(2).getType()));
								params.put("third_level", String.valueOf(systemData.getBuildAt(2).getLevel()));
							}
							else
							{
								params.put("third_id","");
					...
					case 2 :
						...
						params.put("queue_id", String.valueOf(systemData.getBuildAt(1).getType()));
						params.put("queue_level", String.valueOf(systemData.getBuildAt(1).getLevel()));
						params.put("third_id", buildingUI.type);
						params.put("third_level", String.valueOf(buildingUI.level));
						break;
				}
			}
	...
	public void setVisible(boolean visible) {
		...
		Client.getInstance().getTutorial().setLesson(Tutorial.LESSON_SYSTEM_BUILD);
	}
	...
	private void updateBuildingsList() {
		...
		for (String building : FILTERS[filter]) {
			if (building.equals(BuildingData.CORPORATIONS) ||
					building.equals(BuildingData.RESEARCH_CENTER) ||
					building.equals(BuildingData.REFINERY) ||
					building.equals(BuildingData.TRADE_PORT) ||
					building.equals(BuildingData.EXTRACTOR_CENTER) ||
					building.equals(BuildingData.FACTORY)) {
				// Bâtiments qui ne peuvent être construits qu'une fois
				boolean found = false;
				
				for (int i = buildSlot - 1; i >= 0; i--)
					if (systemData.getBuildAt(i).getType().equals(building)) {
						if (systemData.getBuildAt(i).getLevel() != 4 &&
								filter != FILTER_LEVEL_1 && isBuildingUnlocked(
								building, systemData.getBuildAt(i).getLevel() + 1))
							buildings.add(new BuildingUI(building,
								systemData.getBuildAt(i).getLevel() + 1));
						found = true;
						break;
					}
				...
				if (!found)
					for (int level = 0; level < 5; level++)
						if (systemData.getBuildingsCount(building, level) > 0) {
							if (level != 4 && filter != FILTER_LEVEL_1 &&
									isBuildingUnlocked(building, level + 1))
								buildings.add(new BuildingUI(building, level + 1));
							found = true;
							break;
						}
				
				if (!found && filter != FILTER_UPGRADES &&
						isBuildingUnlocked(building, 0))
					buildings.add(new BuildingUI(building, 0));
			} else {
				if (filter != FILTER_UPGRADES &&
						isBuildingUnlocked(building, 0))
					buildings.add(new BuildingUI(building, 0));
				
				if (filter != FILTER_LEVEL_1) {
					for (int level = 1; level < 5; level++) {
						int previousLevelBuildingsCount =
							systemData.getBuildingsCount(building, level - 1);
						
						for (int i = 0; i < buildSlot; i++) {
							if (systemData.getBuildAt(i).getType().equals(building)) {
								if (systemData.getBuildAt(i).getLevel() == level)
									previousLevelBuildingsCount--;
								else if (systemData.getBuildAt(i).getLevel() == level - 1)
									previousLevelBuildingsCount++;
							}
						}
						
						if (previousLevelBuildingsCount > 0 &&
								isBuildingUnlocked(building, level))
							buildings.add(new BuildingUI(building, level));
					}
				}
			}
		}
		...
		public String toString() {
			if (type == null) {
				return "<table unselectable=\"on\" class=\"action\" " +
					...
					"style=\"padding: 2px 5px;\" class=\"small\"><div class=\"title\" " +
					"unselectable=\"on\">Aucun bâtiment</div>" +
					"</td></tr></table>";
			}
			...
			String cost = "<div class=\"small right\" style=\"float: right;\" " +
					"unselectable=\"on\">";
			int creditsCost = (int) Math.ceil(buildingCost[4] *
				BuildingData.getProduction(BuildingData.TRADE_PORT, systemData));
			...
			for (int i = 0; i < 4; i++) {
				...
				long currentResource =
					resourcesManager.getCurrentResource(systemData.getId(), i);
			...
			if (level == 0) {
				if (type.startsWith(BuildingData.EXPLOITATION)) {
					// Gisements de ressources disponibles
					int inUse = 0;
					for (int i = 0; i < 5; i++)
						inUse += systemData.getBuildingsCount(type, i);
					for (int i = 0; i < systemData.getBuildsCount(); i++)
						inUse += systemData.getBuildAt(i).getType().equals(type) &&
						systemData.getBuildAt(i).getLevel() == 0 ? 1 : 0;
					
					int resourceIndex = Integer.parseInt(
						type.substring(BuildingData.EXPLOITATION.length()));
					
					deposit = "<div class=\"emphasize\" style=\"clear: both;\">" +
						"<span style=\"float: right;\">" +
						(systemData.getAvailableResource(resourceIndex) - inUse) +
						" / " + systemData.getAvailableResource(resourceIndex) +
						" " + Utilities.parseSmilies(":r" + (resourceIndex + 1) + ":") + "</span>" +
						"Gisements disponibles</div>";
					
					if (systemData.getAvailableResource(resourceIndex) <= inUse)
						available = false;
					...
					deposit = "<div class=\"emphasize\" style=\"clear: both;\">" +
						...
						" / " + systemData.getAvailableSpace() +
						"</span>Espace disponible</div>";
					
					if (systemData.getAvailableSpace() <= inUse)
						available = false;
				}
			}
			
			String desc = cost + "<div class=\"title\" unselectable=\"on\">" +
				BuildingData.getName(type, level) + "</div>" +
				"<div class=\"x-small\" unselectable=\"on\" style=\"padding-right: 80px;\">" +
				BuildingData.getDesc(type, level, level > 0) + deposit + "</div>";
			
			return "<table unselectable=\"on\"" + (available ? "" :
				" class=\"unavailable\"") +
				"cellspacing=\"0\" style=\"width: 100%;\">" +
				"<tr unselectable=\"on\">" +
				"<td unselectable=\"on\" valign=\"top\" style=\"width: 54px;\">" +
				"<div unselectable=\"on\" class=\"control " +
				BuildingData.getClassName(type) + (available ? "" : " disabled") +
				"\"></div></td><td unselectable=\"on\" valign=\"center\" " +
				"style=\"padding: 2px 5px;\" class=\"small\">" + desc + "</td></tr></table>";
		}
