...
public class JSOptionPane extends JSDialog implements DialogListener,
		ClickListener {
	// ------------------------------------------------------- CONSTANTES -- //
	...
	protected JSOptionPane(String message, String title, int options,
			String type, OptionPaneListener listener, String[] values,
			String defaultValue, boolean input, String inputText,
		...
		HTMLPanel contentPanel = new HTMLPanel(
			"<table class=\"standard " + type + "\" unselectable=\"on\">" +
			"<tr unselectable=\"on\">" +
			"<td class=\"icon\" unselectable=\"on\">" +
			"<div unselectable=\"on\"></div></td>" +
	...
	public void dialogClosed(Widget sender) {
		if (listener != null) {
			if (inputField != null || valuesComboBox != null)
				listener.optionSelected(null);
			else
				listener.optionSelected(CLOSED_OPTION);
		}
		cleanUp();
	}
	...
	public void onClick(Widget sender) {
		if (listener != null) {
			if (inputField != null) {
				if (sender == yesBt || sender == okBt)
					listener.optionSelected(inputField.getText());
				else if (sender == noBt || sender == cancelBt)
					listener.optionSelected(null);
			} else if (valuesComboBox != null) {
				if (sender == yesBt || sender == okBt)
					listener.optionSelected(valuesComboBox.getSelectedItem().toString());
				else if (sender == noBt || sender == cancelBt)
					listener.optionSelected(null);
			} else {
				if (sender == yesBt)
					listener.optionSelected(YES_OPTION);
				else if (sender == noBt)
					listener.optionSelected(NO_OPTION);
				else if (sender == okBt)
					listener.optionSelected(OK_OPTION);
				else if (sender == cancelBt)
					listener.optionSelected(CANCEL_OPTION);
			}
		}
	...
	public static void showMessageDialog(String message, String title,
			int options, String type, OptionPaneListener listener) {
		new JSOptionPane(message, title, options, type, listener, null, null,
				false, null, false);
	}
	
	public static void showOptionDialog(String message, String title,
			int options, String type, OptionPaneListener listener,
			String[] values, String defaultValue) {
		new JSOptionPane(message, title, options, type, listener, values,
				defaultValue, false, null, false);
	}

	public static void showInputDialog(String message, String title,
			int options, String type, OptionPaneListener listener,
			String inputText) {
		new JSOptionPane(message, title, options, type, listener, null, null,
				true, inputText, false);
	}
	
	public static void showInputDialog(String message, String title,
			int options, String type, OptionPaneListener listener,
			String inputText, boolean passwordField) {
		new JSOptionPane(message, title, options, type, listener, null, null,
				true, inputText, passwordField);
	}
