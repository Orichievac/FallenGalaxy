...
public class IndexedAreaData {
	...
	public IndexedAreaData(AreaData areaData) {
		...
		for (int i = 0; i < areaData.getHyperspaceSignaturesCount(); i++)
			hyperspaceSignaturesMap.put(areaData.getHyperspaceSignatureAt(i).getId(),
				areaData.getHyperspaceSignatureAt(i));
		
		this.asteroidsMap = new HashMap<Integer, AsteroidsData>();
		for (int i = 0; i < areaData.getAsteroidsCount(); i++)
			asteroidsMap.put(areaData.getAsteroidsAt(i).getId(),
				areaData.getAsteroidsAt(i));
		
		this.structuresMap = new HashMap<Long, StructureData>();
		for (int i = 0; i < areaData.getStructuresCount(); i++)
			structuresMap.put((long) areaData.getStructureAt(i).getId(),
				areaData.getStructureAt(i));
		
		this.gravityWellsMap = new HashMap<Integer, GravityWellData>();
		for (int i = 0; i < areaData.getGravityWellsCount(); i++)
			gravityWellsMap.put(areaData.getGravityWellAt(i).getId(),
				areaData.getGravityWellAt(i));
	}
	...
	public String getEnvironment() {
		return areaData.getEnvironment();
	}
	...
	public int getLotterysCount() {
		return areaData.getLotterysCount();
	}
	
	public LotteryData getLotteryAt(int index) {
		return areaData.getLotteryAt(index);
	}
